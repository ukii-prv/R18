...
function handleCelebrateButtonClick() {
  ...
  state.flowerSizeBoost = Math.min(4.2, state.flowerSizeBoost === 0 ? 0.18 : state.flowerSizeBoost * 1.18);
  state.flowerSpinBoost = Math.min(3.6, state.flowerSpinBoost + 0.18);
  startFlowerStream();
  setCelebrateButtonVisible(false);
  window.setTimeout(() => {
...
function updateFlowerParticles(deltaSeconds) {
  ...
  const maxParticles = 220 + Math.round(state.flowerStreamIntensity * 42);
...
function spawnFlowerParticle(isBurst) {
  ...
  const sizeMultiplier = 1 + state.flowerSizeBoost;
  const size = baseSize * sizeMultiplier;
  const life = (isBurst ? 4.2 : 4.6) + random() * 2.0;
  const spinMultiplier = 1 + state.flowerSpinBoost;
  ...
  system.particles.push({
    ...
    rotationStart: (-0.24 + random() * 0.48) * spinMultiplier,
    rotationDelta: (-0.65 + random() * 1.3) * spinMultiplier,
    rotation: 0,
    alpha: 0
  });
}
